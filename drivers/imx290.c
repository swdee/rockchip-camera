...
static int __maybe_unused imx290_get_selection(struct v4l2_subdev *sd,
				struct v4l2_subdev_state *sd_state,
...
/*
 * Rockchip's downstream CIF/D-PHY stack queries these operations while
 * discovering the terminal sensor.
 */
static int imx290_get_frame_interval(struct v4l2_subdev *sd,
				     struct v4l2_subdev_frame_interval *fi)
{
	...
	/*
	 * Both modes in this driver default to 30 fps:
	 *   1920x1080: HMAX 2200, VMAX 1125
	 *   1280x720:  HMAX 3300, VMAX 750
	 * with a 148.5 MHz pixel clock.
	 */
				  ...
				  struct rkmodule_inf *inf)
{
	...
	strscpy(inf->base.sensor, imx290->model->name,
		sizeof(inf->base.sensor));
	strscpy(inf->base.module, imx290->module_name,
		sizeof(inf->base.module));
	strscpy(inf->base.lens, imx290->len_name,
...
static long imx290_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_GET_MODULE_INFO:
		imx290_get_module_inf(imx290, arg);
		return 0;
	default:
		return -ENOIOCTLCMD;
	}
...
static const struct v4l2_subdev_video_ops imx290_video_ops = {
	...
	.g_frame_interval = imx290_get_frame_interval,
};
...
static int imx290_probe(struct i2c_client *client)
{
	...
	ret |= of_property_read_string(dev->of_node,
				       ...
				       &imx290->len_name);
	if (ret) {
		dev_err(dev, "Could not get Rockchip module information\n");
		return -EINVAL;
	}
...

	{
		...
		facing[0] = !strcmp(imx290->module_facing, "back") ? 'b' : 'f';

		snprintf(imx290->sd.name, sizeof(imx290->sd.name),
			 "m%02d_%s_%s %s", imx290->module_index, facing,
			 imx290->model->name, dev_name(imx290->sd.dev));
	}
...
MODULE_DESCRIPTION("Sony IMX290 CMOS Image Sensor Driver with Rockchip RKAIQ support");
